 ...
 * 앞의 문제에 대해서는 답을 틀리다가 답이 맞는 처음 문제는 1점으로 계산한다. 또한, 연속으로 문제의 답이 맞는 경우에서 두 번째 문제는 2점, 세 번째 문제는 3점, ..., K번째 문제는 K점으로 계산한다.
 * 틀린 문제는 0점으로 계산한다.
...
function solution(arr) {
  return arr.reduce(
    (acc, cur, i) => {
      acc.accAnswer = cur ? acc.accAnswer + 1 : 0;
      acc.total += acc.accAnswer;
      if (i === arr.length - 1) return acc.total;
      return acc;
    },
    { total: 0, accAnswer: 0 }
  );
